...
This is likely to perform with low latency on EC2 with instance storage or EBS. Non-scientific tests showed it worked better than expected on an EFS mount. If additional latency can be tolerated, the _bargain basement_ solution is to read the **individual record** from S3.
...
A simple remedy is to direct the add/remove/change _commands_ to individual files, a sort of write-ahead log. At a timed interval, a single compaction process could run and merge these changes into the ordered index file - discarding the temporary files after the changes have been _committed_. This reduces the amount of work needed to perform a sort and rewrite on the entire index, particularly if networked storage or S3 is where the indexes are stored.

If a degree of latency is acceptable in changes appearing, this is not a bad trade off. Another hack would be to not consult the pagination index at all when querying the first n pages, and simply limit in your client. For example, instead of setting the DynamoDB query to `20`, set it to `200` and take a slice of the returned items to deliver up to page 10. This will increase read costs but caters for newest always being visible.
...
A sparsely populated GSI would use this attribute as its sort key (plus other keys) so that only page _start_ items are included. A partition header item would contain the current page number (ascending) and running count of items remaining for the current page. If the new record flows onto the next page (i.e. is record 20), a page marker attribute is added to it and the statistics values incremented/reset within a transaction. 
