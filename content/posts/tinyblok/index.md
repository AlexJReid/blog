...
Right now it's an ESP32-C6 that brings up Wi-Fi, opens a TCP socket to a NATS broker, sends `CONNECT`, and publishes the on-die temperature sensor reading once a second. C owns everything that touches ESP-IDF (Wi-Fi, NVS, lwIP, the NATS client); Zig owns the sample-and-publish loop, kept dependency-free so the patchbay runtime can land there next.
...
**The C NATS client is hand-rolled.** The obvious off-the-shelf options didn't fit. Synadia's [nats.c](https://github.com/nats-io/nats.c) is a good library on a desktop, but it pulls in pthreads, a thread pool, and TLS through OpenSSL/MbedTLS, none of which is a good match for an ESP-IDF FreeRTOS app where the NATS client is one of several tasks sharing 320 KB of RAM. Same story for [nats.zig](https://github.com/nats-io/nats.zig): assumes `std.Io.Threaded` and `std.crypto.tls`, neither of which exist in any meaningful sense on bare-metal RISC-V. So a small bespoke client it is: this is the beauty of the NATS protocol: the wire format is so simple you can implement the publish-only subset in a small amount of C and have it talk to a real broker. TLS and auth is a problem for another day, but doable.
