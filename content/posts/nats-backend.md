...
tags = ['nats', 'websocket', 'backend', 'greatest-hits']
categories = []
externalLink = ""
series = []
+++
...
Bringing in some even older, historic data might also be useful. NATS provides the ability for clients to make _requests_ via a subject that a service is listening to. The service, if available, publishes one or more _responses_ to a temporary inbox subject that the client is listening to. A backend service could be implemented to proxy queries to a database. Large result sets can be chunked over multiple messages to the request inbox. A zero byte payload signals that there are no more chunks.
...
> For internal apps with a handful of users, high server side resource consumption and significantly higher network usage might be a worthy trade-off. Some of the benefits of using NATS as a backend still apply.
