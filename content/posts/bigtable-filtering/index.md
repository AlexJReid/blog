...
It is worth pointing out that unlike DynamoDB, Cloud Bigtable is a poor fit for small amounts of data. It has a fairly high entry-level cost and optimizations that it can only perform at scale, making it overkill in many scenarios. That said, some of the modelling techniques presented here will apply to larger volume and velocity usage scenarios where Cloud Bigtable makes sense.

I am not suggesting you actually use Cloud Bigtable to build a product comments system, unless it was to be at a massive scale. Use DynamoDB, Firebase, MySQL or one of the many alternatives. **However** ... if _comments as a service_ was your primary business offering, Cloud Bigtable could start to make more sense. As would DynamoDB, of course. The point is that DynamoDB can support workloads of varying sizes.
...
Despite this simple interface, Bigtable is an abstraction on top of cloud storage, infrastructure and workload orchestration genius.
...
Although simple to implement, this approach will have worse maintenance overhead compared to the DynamoDB version. There is no equivalent of DynamoDB Streams to create the projections in an event-driven manner, meaning this work will be pushed out to the client program.

It would be possible to accept changes into a PubSub topic and have Cloud Functions or Cloud Run perform the Cloud Bigtable mutations. This has the drawback of making comment submission fire and forget: the comment will be accepted onto the topic, but will not be confirmed as having been written to the table. Whether this is acceptable depends on the application.

### Regular expression row filter

Within reason, filters in Bigtable are maybe more acceptable than with DynamoDB. **The big difference with Bigtable is the fact that the lowest possibly amount of capacity is a single node. It is a _sunk cost_, reading tens of thousands of rows maybe does not matter, when a single node is capable of scanning up to 220mb/sec.** Of course, this inefficiency might catch up with us as volume and concurrent access increases. Node resources are not infinite and scanning excessively will make query performance less predictable.
...
Bigtable has no notion of atomicity between writes, beyond those to a single row. There is a chance that a comment fails to appear in all indexes.
...
- Read rows from `P#42/en/1` to `P#42/en/1\xff`
- Read rows from `P#42/en/2` to `P#42/en/2\xff`
- Read rows from `P#42/en/5` to `P#42/en/5\xff`

Merge results, sort by reverse timestamp, return topN. The approach for paging is the same taken for DynamoDB and is [explained in this post](/posts/dynamodb-efficient-filtering-3/).
